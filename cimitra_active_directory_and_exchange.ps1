...
$UserSearchReturn = Check-For-SamAccountName-Exclusions-Silent($SamAccountName)
    ...
    if($UserSearchReturn -eq 1){
		Write-Output "--------------------------------------------------------------------------------"
        Write-Output "Insufficient Rights to Display Matching User"
        Write-Output "--------------------------------------------------------------------------------"
        Write-Output ""
        if($ShowStandardError){
            $Host.UI.WriteErrorLine("Insufficient Rights to Display Matching User(s)")  
         }
...
if($USER_LIST_ONE_LENGTH -gt 0)
{
    ...
    if($UserSearchReturn -eq 1){
        ...
        if($ShowStandardError){
            $Host.UI.WriteErrorLine("Insufficient Rights to Display Matching User")  
         }
