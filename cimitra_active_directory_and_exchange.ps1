            ...
            if(!($ExcludeGroupEmpty)){
                ...
                if($SkipUserSamAccountNameDisplay){
                        ...
                        if(!( $ShowDistinguishedName)){
                                Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -Searchbase $contextIn  -ErrorAction Stop | select Givenname,Surname  | Format-Table -HideTableHeaders
                         }else{
                                Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -Searchbase $contextIn  -ErrorAction Stop | select Givenname,Surname,DistinguishedName  | Format-Table -HideTableHeaders
                         }
                    }else{

                         if(!($ShowDistinguishedName)){
                                Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -Searchbase $contextIn  -ErrorAction Stop | select Givenname,Surname,SamAccountName  | Format-Table -HideTableHeaders
                            }else{
                                Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -Searchbase $contextIn  -ErrorAction Stop | select Givenname,Surname,SamAccountName,DistinguishedName  | Format-Table -HideTableHeaders
                            }
                }
        ...
        try{
                if(!($ExcludeGroupEmpty)){
                        ...
                        if($SkipUserSamAccountNameDisplay){
                                ...
                                if(!($ShowDistinguishedName)){
                                    Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -ErrorAction Stop | select Givenname,Surname  | Format-Table -HideTableHeaders
                                }else{
                                    Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -ErrorAction Stop | select Givenname,Surname,DistinguishedName  | Select @{Name="First Name";Expression={$_.GivenName}},@{Name="Last Name";Expression={$_.Surname}},@{Name="Distinguished Name";Expression={$_.DistinguishedName}} | fl
                                }
                        }else{
                                if(!($ShowDistinguishedName)){
                                        Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -ErrorAction Stop | select Givenname,Surname,SamAccountName  | Format-Table -HideTableHeaders
                                    }else{
                                        Get-ADUser -Filter { memberof -ne $AD_EXCLUDE_GROUP } -ErrorAction Stop | select Givenname,Surname,SamAccountName,DistinguishedName  | Select @{Name="First Name";Expression={$_.GivenName}},@{Name="Last Name";Expression={$_.Surname}},@{Name="UserID";Expression={$_.SamAccountName}},@{Name="Distinguished Name";Expression={$_.DistinguishedName}} | fl
                                    }
                        }
                    }else{
                        if($SkipUserSamAccountNameDisplay){
