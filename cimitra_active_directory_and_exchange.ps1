...
# echo "DD1"
        ...
        #$global:SamAccountNameIn = $SAM
# echo "SamAccountName = $SamAccountNameIn"
# Get-ADUser -Filter { SamAccountName -eq $SamAccountNameIn } -SearchBase "$UserContextIn" 
# exit 0
...
$UserSearchReturn = Check-For-SamAccountName-Exclusions-Silent($SamAccountName)
    ...
    if($UserSearchReturn -eq 1){
		Write-Output "--------------------------------------------------------------------------------"
        Write-Output "Insufficient Rights to Administer Matching User(s)"
        Write-Output "--------------------------------------------------------------------------------"
        Write-Output ""
        if(!($SuppressStandardError)){
            $Host.UI.WriteErrorLine("Insufficient Rights to Administer Matching User(s)")  
         }
...
if(!($USER_LIST_ONE_LENGTH -match "^\d+$"))
{
$UseAlternativeNumber = $true
}
...
Write-Output "-------------------------"
        ...
        if(!($UseAlternativeNumber)){
            Write-Output ""
            Write-Output "$USER_LIST_ONE_LENGTH USERS MATCH THE SEARCH"
            Write-Output "--------------------------"
        }else{
            Write-Output ""
            Write-Output "1 USER MATCHES THE SEARCH"
...
if($USER_LIST_ONE_LENGTH -gt 0)
{
    ...
    while($COUNTER -lt $USER_LIST_ONE_LENGTH){
        ...
        # See if the user is in the Exlude Group

        $UserSearchReturn = Check-For-SamAccountName-Exclusions-Silent($TheSamAccountName)

    if($UserSearchReturn -eq 1){
		Write-Output "--------------------------------------------------------------------------------"
        Write-Output "Insufficient Rights to Administer Matching User"
        Write-Output "--------------------------------------------------------------------------------"
        Write-Output ""

        if(!($SuppressStandardError)){
            $Host.UI.WriteErrorLine("Insufficient Rights to Administer Matching User")  
         }
        ...
        $TheSamAccountName = $USER_LIST_ONE.SamAccountName
        Write-Output "[ USER #1 ] INFO START"
        Write-Output "----------------------"
        #Write-Output "SamAccountName: $TheSamAccountName"
        $global:SamAccountNameIn = $TheSamAccountName
        Postiviely-Identify-User
        Get-User-Info
        Write-Output "[ USER #1 ] INFO FINISH"
        Write-Output "-----------------------"
