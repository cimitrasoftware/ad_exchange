...
$global:SamAccountName = "$SamAccountName"
...
$global:TheDefaultPasswordIn = $DefaultPassword
}
...
$ExcludeGroupGUID = $ExcludeGroupGUID
}
...
# Modify the user
try{
Set-ADUser -Identity $SamAccountName -SamAccountName $NewSamAccountName 2>$null
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
{
Write-Output ""
Write-Output "User: ${UserFirstName} ${UserLastName} | Userid: $SamAccountName Changed To: $NewSamAccountName"
$global:SamAccountName = $NewSamAccountName
}else{
Write-Output ""
Write-Output "User: ${UserFirstName} ${UserLastName} | Userid: $SamAccountName NOT Changed To: $NewSamAccountName"
Write-Output ""
    if ($verboseOutputSet){
    Write-Output "[ERROR MESSAGE BELOW]"
    Write-Output "-----------------------------"
...
# Modify the user
try{
Get-ADUser -Identity $SamAccountName  | Rename-ADObject -NewName "${NewFirstName} ${UserLastName}" -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Get-ADUser -Identity $SamAccountName  | Set-ADUser -DisplayName "${NewFirstName} ${UserLastName}" -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Get-ADUser -Identity $SamAccountName  | Set-ADUser -GivenName ${NewFirstName} -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
# Modify the user
try{
Get-ADUser -Identity $SamAccountName  | Rename-ADObject -NewName "${UserFirstName} ${NewLastName}" -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Get-ADUser -Identity $SamAccountName  | Set-ADUser -DisplayName "${UserFirstName} ${NewLastName}" -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Get-ADUser -Identity $SamAccountName  | Set-ADUser -Surname ${NewLastName} -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Set-ADUser -Identity $SamAccountName -manager $ManagerSamAccountName -ErrorAction Stop  2>$null
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Set-ADUser -Identity $SamAccountName  -Manager "CN=${ManagerFirstName} ${ManagerLastName},${ManagerContext}" -ErrorAction Stop 2>$null
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
$UserSearchReturn = SearchForUserSamAccountName "$ManagerFirstName" "$ManagerLastName"
...
# Write-Output "Set-ADUser -Identity $SamAccountName -AccountExpirationDate $ExpirationDate"

$modifyUserResult = $true

# Modify the user
try{
Set-ADUser -Identity $SamAccountName -AccountExpirationDate $ExpirationDate -ErrorAction Stop
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
if($DescriptionSet){
    ...
    if(!($AddToExchange)){
    UpdateCreatedUserProperty $SamAccountName "Description" "$Description" "Description"
    }else{
...
if($DepartmentNameSet){
    ...
    if(!($AddToExchange)){
    UpdateCreatedUserProperty $SamAccountName "Department" "$DepartmentName" "Department Name"
    }else{
...
if($TitleSet){
    ...
    if(!($AddToExchange)){
    UpdateCreatedUserProperty $SamAccountName "Title" "$Title" "Title"
    }else{
...
if($MobilePhoneSet){
    ...
    if(!($AddToExchange)){
    UpdateCreatedUserProperty $SamAccountName "MobilePhone" "$MobilePhone" "Mobile Phone"
    }else{
...
if($OfficePhoneSet){
    ...
    if(!($AddToExchange)){
    UpdateCreatedUserProperty $SamAccountName "OfficePhone" "$OfficePhone" "Office Phone"
    }else{
